...
const { data: posts } = await useAsyncData('posts:archives', () =>
  ...
  <section>
    ...
    <div v-if="groups.length" class="mt-4 space-y-6">
      <section v-for="[month, list] in groups" :key="month">
        <h2 class="mb-2 text-base font-semibold text-slate-900">{{ month }}</h2>
        <ul class="divide-y divide-slate-100">
          <li v-for="post in list" :key="post.path" class="flex items-baseline gap-3 py-3">
            <span v-if="post.date" class="min-w-[84px] text-xs text-slate-500">{{ post.date }}</span>
            <NuxtLink class="text-slate-900 no-underline hover:underline" :to="post.path">
