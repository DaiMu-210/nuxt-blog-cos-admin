...
async function validateCoscliBinary(filePath) {
  ...
  if (process.platform === 'darwin') {
    ...
    const macho = new Set([0xfeedface, 0xfeedfacf, 0xcafebabe, 0xcafebabf, 0xcefaedfe, 0xcffaedfe, 0xbebafeca, 0xbfbafeca])
...
function getDefaultDownloadCandidates() {
  ...
  if (platform === 'darwin' && arch === 'x64') {
    return [
      `${base}/coscli-darwin-amd64`,
      `${base}/coscli-macos-amd64`,
      `${base}/coscli-mac-amd64`,
    ]
  }

  if (platform === 'darwin' && arch === 'arm64') {
    return [
      `${base}/coscli-darwin-arm64`,
      `${base}/coscli-macos-arm64`,
      `${base}/coscli-mac-arm64`,
    ]
  }
...
async function downloadFile(url, destPath) {
  ...
  const res = await new Promise((resolveRes, reject) => {
...
export async function ensureCoscli() {
  ...
  throw new Error(`自动下载 coscli 失败。可改用本机安装（PATH），或设置 COSCLI_DOWNLOAD_URL。${detail ? `\n${detail}` : ''}`)
}
